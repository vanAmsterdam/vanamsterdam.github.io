...
- macm1: 2020 macbook air M1, 8Gb, 8 threads
- linux: linux machine with 12 threads (Intel(R) Xeon(R) W-2135 CPU @ 3.70GH )
...
tw <- dcast(timings[language!='jax'], language+nreps~machine+nthreads, value.var='n_per_min3')
...
tabcols <- c('nreps', 'nthreads', 'min_total', 'n_per_min3', 'vs_r_speedup', 'language', 'machine')
...
1. julia and Jax where faster than R:
  - julia: 3.3x faster on 8-thread Mac M1, 9.6x faster on 12-thread linux machine
  - Jax: 1.7x faster on both machines
2. julia 
