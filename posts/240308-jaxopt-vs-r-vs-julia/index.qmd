...
First, let's see how speed scales with the number of repetitions
...
if (file.exists('bashtimings.txt')) {
  ...
  timings <- rbindlist(list(
    macm1=timings,
    linux=btimings
  ), idcol="machine", fill=T)
}
...
ggplot(timings[nthreads==max_threads], aes(x=nreps, y=min_total, col=language)) +
  ...
  ggtitle(paste0("time to run experiments with ", max_threads, " threads")) + 
  facet_grid(machine~.)
```
...
ggplot(timings[nthreads==max_threads], aes(x=nreps, y=n_per_min, col=language)) +
  geom_point() + geom_line() + 
  scale_x_log10() + scale_y_log10() + 
  ggtitle(paste0("experiments per minute with ", max_threads, " threads")) + 
  facet_grid(machine~.)
```
...
tabcols <- c('nreps', 'nthreads', 'min_total', 'log_n_per_min', 'language', 'machine')
...
kable(timings[order(-n_per_min), .SD, .SDcols=tabcols][1:5])
```
