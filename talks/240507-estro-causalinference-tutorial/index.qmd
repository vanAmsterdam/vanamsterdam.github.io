...
- formalize model of world with variables (treatment, determinants of treatment, outcome) and *causal relationships* between these variables
- define treatment effect as perfect intervention on a variable (i.e. force treatment = surgery) and calculate outcomes ($\Pr(Y=1|\text{do}(T=1))$)
- can be estimated from data when the right assumptions are met for data available
