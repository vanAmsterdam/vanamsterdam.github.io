...
- Late 2023, thought of an empirical project to test an hypothesis inspired by the paper
...
- If $Y$ is binary, $\Psi$ could be a risk difference, risk ratio ($\Psi_{rr}$), survival ratio ($\Psi_{sr}$), number needed to treat, odds ratio, etc.

## Estimating effects

- We get these estimates from RCTs, as these give us random samples from $P(Y^0),P(Y^1)$:

. . .

  $$D = \sum_{i}^{n_0} Y_i^0 = 1, \sum_{i}^{n_0} Y_i^0 = 0, \sum_i^{n_1} Y_i^1 = 1, \sum_i^{n_1} Y_i^1 = 0 $$

- For a well-conducted RCT, many $\Psi$ are a *valid* measure of the treatment effect
...
- Need to make a decision on interventions for *a specific* subpopulation ('target') with a specific distribution of $X$: $P^*(X)$
...
- note: this is a *random effects* model; for *fixed effects* models, $\tau=0$ is assumed
...
- Need to make assumptions to pick best measure
...
variance in outcome risk in control arms depends more on the distribution of $X$ (or on more $X$s), than the *effect* of the treatment (contrast between treated and control)
:::
